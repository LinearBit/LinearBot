...
public class Config {
    ...
    private static final File formatFile = new File(INSTANCE.getDataFolder(), "format.yml");
    ...
    private static YamlConfiguration format;

    public static void createConfig(){
        File[] allFile = {botFile,configFile,returnsFile,commandsFile,formatFile};
        ...
        if (!Config.getFormatYamlVersion().equals("1.2.2")){
            INSTANCE.saveResource(formatFile.getName(), true);
        }
    }

    private static String getFormatYamlVersion() {
        return getFormatYaml().getString("Ver");
    }
    ...
    public static YamlConfiguration getBotYaml(){return bot;}

    public static YamlConfiguration getConfigYaml() {return config;}
    ...
    private static YamlConfiguration getFormatYaml() {return format;}
    ...
    public static String getFormat(String mode){
        if (mode.equals("QQ")) {
            return getFormatYaml().getString("QQ");
        }
        else {
            return getFormatYaml().getString("Server");
        }
    }
