...
public class QQEvent implements Listener {
    ...
    public void onGroupMessageReceive(MiraiGroupMessageEvent e){
        ...
        String text=Config.getFormat("Server").replace("%groupName%",groupName);


        pattern = Pattern.compile("<?xm.*");
        matcher = pattern.matcher(msg);
        if (matcher.find()){
            msg = "不支持的消息类型，请在群聊中查看";
            text=text.replace("%qq_name%","").replace("%msg%",msg);
        ...
        if (Args.ForwardingMode()==1){
            ...
            text=text.replace("%qq_name%",name).replace("%msg%",smsg);
            Bukkit.broadcastMessage(text);
            return;
        }
